...
df_columns = ["nct_id", "brief_title", "official_title", "conditions_list", "eligibility_criteria", "description", "keywords_list", "pub_med_id"]
...
for input_file in list_of_files:
    ...
    #PmId 8   
    try:
        pm = root.find('reference').find('PMID').text
        data.append(pm.strip().replace("\n", ""))
        pmctr += 1
    except:
        data.append("NA")
...
#     print(data)
    ...
    if ctr%10000 == 0:
        ...
        print("PmId's Found=", str(pmctr), "OutOf: ", str(ctr))
    
    if ctr%50000 == 0:    
        df.to_csv("../output/all_med_19th_jan_" + str(ctr) + ".csv", index=False)

df.to_csv("../output/all_med_19th_jan.csv", index=False)