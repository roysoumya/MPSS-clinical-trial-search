...
def extract_data_xml(nctIds, cur_index):
       """
       ...
       list_of_files = glob.glob('../../clinicalTrial/data/AllPublicXML/*' + '/*.xml')
       # print(list_of_files)
...
def elastic_index(ctr, extracted_data, cur_index):
       ...
       try:
              # print("Index:", index_)
              # print(str(cur_index)+"_"+str(cur_index))
              print(str(cur_index)+"_" + str(cur_index) + "_" + str(cur_index))
              es.index(index=str(cur_index)+"_" + str(cur_index) + "_" + str(cur_index), doc_type='xmldata', id=ctr, body=extracted_data)
...
if __name__ == '__main__':
       ...
       try:
              input_path = "../data/5_Class_Csv_files_status_removed"
              ...
              for file in files[:5]:
                     print(file)
                     cur_index = int(file.split("_")[0])
                     df = pd.read_csv(join(input_path, file))
                     nctIds = df['nct_id'].values
                     print(nctIds.shape)
                     es = elasticsearch.Elasticsearch([{'host': 'localhost', 'port': 9200}])
                     
                     start_time = time.time()
                     extract_data_xml(nctIds, cur_index)
